...
import { IS_DEMO_MODE, streamQuery } from "./lib/agentApi";
...
const API_BASE_URL = IS_DEMO_MODE
  ? "Static demo"
  : import.meta.env.VITE_API_BASE_URL || "Vite /api proxy";
        ...
        <aside className="hidden min-h-0 border-r border-ink/10 bg-[#efe6d8]/85 backdrop-blur lg:flex lg:flex-col">
          <div className="border-b border-ink/10 px-5 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-base font-semibold tracking-[0.02em]">RetailInsight</div>
                <div className="text-xs text-ink/50">retail-insight-agent</div>
              </div>
            </div>
          </div>
        ...
        <main className="flex min-h-0 min-w-0 flex-col overflow-hidden">
          <header className="flex h-16 shrink-0 items-center justify-between border-b border-ink/10 bg-parchment/88 px-4 backdrop-blur lg:px-6">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold text-ink">电商智能问数 Agent</div>
                <div className="truncate text-xs text-ink/45">
                  {IS_DEMO_MODE ? "Static Demo / LangGraph Flow" : "FastAPI SSE / LangGraph"}
                </div>
              </div>
            </div>
            <button
          ...
          <div className="border-t border-ink/10 bg-[#efe6d8]/45 px-4 py-2 text-center text-xs text-ink/45">
            <span className="inline-flex items-center gap-2">
              ...
              {isStreaming ? "运行中" : IS_DEMO_MODE ? "演示模式" : "就绪"}
            </span>
          </div>
          <Composer
            value={draft}
